...
[task_local]
#京东宠汪汪
15 0-23/2 * * * https://raw.githubusercontent.com/LingFeng0918/jd_scripts/master/jd_joy.js, tag=京东宠汪汪, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdcww.png, enabled=true

============Loon===========
[Script]
cron "15 0-23/2 * * *" script-path=https://raw.githubusercontent.com/LingFeng0918/jd_scripts/master/jd_joy.js,tag=京东宠汪汪

============Surge==========
[Script]
京东宠汪汪 = type=cron,cronexp="15 0-23/2 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/LingFeng0918/jd_scripts/master/jd_joy.js

===============小火箭==========
京东宠汪汪 = type=cron,script-path=https://raw.githubusercontent.com/LingFeng0918/jd_scripts/master/jd_joy.js, cronexpr="15 0-23/2 * * *", timeout=3600, enable=true
*/
//by 标哥丶 20220118
...
async function petTask() {
    for (let item of $.getPetTaskConfigRes.datas || []) {
        ...
        if (item['taskType'] === 'ScanMarket') {
            ...
            for (let scanMarketItem of scanMarketList) {
                if (!scanMarketItem.status) {
                    ...
                    if (scanMarketRes.errorMessage) {
                        console.log(scanMarketRes.errorMessage);
                        console.log('黑号了，别撸了');
                        break;
                    }
        ...
        if (item['taskType'] === 'DianQi') {
            const url = `https://jdjoy.jd.com/common/pet/doDQTask?invokeKey=q8DNJdpcfRQ69gIx`;
                ...
                $.get(taskUrl(url), async (err, resp, data) => {
                    try {
                        if (err) {
                            ...
                            console.log(`${$.name} doDQTask API请求失败，请检查网路重试`)
                        } else {
                            data = JSON.parse(data);
                            const res = await getFood('DianQi');
                            console.log(`领取京东电器狗粮结果：${res.data}`);
                        }
                    } catch (e) {
                        $.logErr(e, resp);
                    }
...
function doScanMarket(type, body) {
    return new Promise(resolve => {
        const url = `https://draw.jdfcloud.com/common/pet/icon/click?iconCode=${type}&linkAddr=${body}&invokeKey=q8DNJdpcfRQ69gIx&reqSource=h5`
        $.get(taskUrl(url), (err, resp, data) => {
            try {
                if (err) {
                    console.log(JSON.stringify(err))
...
function enterRoom() {
    return new Promise(resolve => {
        const url = `https://api.m.jd.com/api?functionId=petEnterRoom&appid=jdchoujiang_h5&body={%22invitePin%22:%22%22,%22reqSource%22:%22h5%22}`
        $.post(PostUrl(url, { "invitePin": "", "reqSource": "h5" }), (err, resp, data) => {
            try {
                if (err) {
                    console.log(JSON.stringify(err))
...
async function appGetPetTaskConfig() {
    let params = {
        code: '922a5',
        functionId: 'petGetPetTaskConfig',
        body: {reqSource:"weapp"}
    }
    h5ts = await getH5st(params);
    return new Promise(resolve => {
        let time = h5ts.split(';').pop();

        const url = `https://api.m.jd.com/api?client=Android&clientVersion=10.3.3&appid=jdchoujiang_h5&t=${time}&functionId=petGetPetTaskConfig&body={%22reqSource%22:%22weapp%22}&h5st=${encodeURIComponent(h5ts)}`
        $.post(PostUrl(url, { "reqSource": "weapp" }), (err, resp, data) => {
...
async function getPetTaskConfig() {
    let params = {
        code: '922a5',
        functionId: 'petGetPetTaskConfig',
        body: {reqSource:"h5"}
    ...
    return new Promise(resolve => {
        ...
        const url = `https://api.m.jd.com/api?client=android&clientVersion=10.3.3&appid=jdchoujiang_h5&t=${tiem}&functionId=petGetPetTaskConfig&body={%22reqSource%22:%22h5%22}&h5st=${encodeURIComponent(h5ts)}`
        $.post(PostUrl(url, { "reqSource": "h5" }), (err, resp, data) => {
            try {
                if (err) {
                    console.log(JSON.stringify(err))
...
function getSHA256(key, params, dete) {
    ...
    let stringSign = `appid:jdchoujiang_h5&body:${SHA256}&client:android&clientVersion:10.3.3&functionId:${params.functionId}&t:${dete}`
